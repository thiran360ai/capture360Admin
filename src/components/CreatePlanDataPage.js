...
const CreatePlanDataPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://4aae-157-49-242-245.ngrok-free.app/building/plan_details/", {
        method: "POST",
        body: formData,
      });
      if (response.ok) {
  ...
  return (
    <Card style={{ maxWidth: 1000, margin: "auto", marginTop: 30 ,width: '380px'}}>
      <CardContent style={{width: '360px',boxShadow: "0 8px 16px rgba(0,0,0,0.2)",backgroundColor:'#efebe9',borderRadius: 8}}>
        <Typography variant="h6" gutterBottom style={{ marginBottom: 30, color: "##00509e", textAlign: "center", 
        ...
        <form onSubmit={handleSubmit}>
          ...
          <label htmlFor="upload-image">
            ...
            style={{ backgroundColor: "#00509e", color: "#ffffff", margin: "10px 0", borderRadius: 4 }}>
              Upload Image
            </Button>
          </label>
          <Button type="submit" variant="contained" color="primary" style={{ backgroundColor: "#00509e", 
