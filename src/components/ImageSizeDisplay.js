...
const ImageGalleryComponent = () => {
    ...
    const fetchImages = async () => {
      try {
        const response = await fetch(
          "https://6081-2409-408d-3e91-34f1-cc8f-590c-8f98-7c3b.ngrok-free.app/building/video/2/frames/",
          {
            headers: {
  ...
  const renderImage = (imageObj, name, index) => {
    const url = `https://6081-2409-408d-3e91-34f1-cc8f-590c-8f98-7c3b.ngrok-free.app${imageObj.url}`;
          ...
          onLoad={() => {
            ...
            if (canvas && imageRefs[index].current) {
              ...
              if (circles[currentIndex + index]) {
                circles[currentIndex + index].forEach((circle) => drawCircle(ctx, circle.x, circle.y));
              }
            }
          }}
        />
    ...
    <div style={{ textAlign: "center" }}>
      ...
      {images.length > 0 ? (
        ...
        >
          ...
          >
            {renderImage(images[currentIndex], `360 Degree Image ${currentIndex}`, 0)}
            <Button
              variant="contained"
              color="primary"
              onClick={handlePrev}
          ...
          {isSplit && (
            ...
            >
              {renderImage(images[(currentIndex + 1) % images.length], `360 Degree Image ${(currentIndex + 1) % images.length}`, 1)}
            </div>
          )}
        </div>
      ) : (
